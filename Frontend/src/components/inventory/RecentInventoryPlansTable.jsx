...
export default function RecentInventoryPlansTable({ rows }) {
  if (!rows.length) {
    return (
      <p className="py-6 text-center text-sm text-slate-500">
        No saved inventory plans yet. Generate a plan to persist it via the API.
      </p>
    )
  }
      ...
      <table className="w-full min-w-[560px] text-sm">
        <thead>
          <tr className="border-b border-slate-200 text-left text-xs uppercase tracking-wider text-slate-500 dark:border-slate-700">
            ...
            <th className="pb-3 pr-4 font-medium">Inventory Cost</th>
        ...
        <tbody>
          {rows.map((row) => (
            ...
            >
              ...
              <td className="py-3 pr-4">{formatNumber(row.ingredients)}</td>
